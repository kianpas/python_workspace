    ...
    if len(data) > 0:
        ...
        if len(cond_result) > 0:
            ...
            for num in range(int(leng)):
                var1[f'selector{num+1}'] = cond_result[f"선택자{num+1}"].item()
                var2[f'name{num+1}'] = cond_result[f"수집항목{num+1}"].item()
                
            print(var1)
            url = st.text_input("URL")
            # selector1 = cond_result["선택자1"].item()
            # # print(cond_result.get("분류코드").values[0])
            # selector2 = cond_result.get("선택자2").values[0]
        ...
        if collect_btn:
            ...
            for num in range(len(var1)):
                result_var[f'result{num+1}'] = soup.select(var1[f"selector{num+1}"])[0]["value"] if soup.select(var1[f"selector{num+1}"]) is not None else ""
